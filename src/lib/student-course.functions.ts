...
async function assertEnrolled(studentId: string, courseId: string) {
  const { data } = await (await admin()).from("enrollments")
...
async function recomputeProgress(studentId: string, courseId: string) {
  ...
  const { data: sections } = await (await admin()).from("sections").select("id").eq("course_id", courseId);
  const sectionIds = (sections ?? []).map((s) => s.id);
  if (!sectionIds.length) return;
  const { data: lessons } = await (await admin()).from("lessons").select("id").in("section_id", sectionIds);
  ...
  await (await admin()).from("enrollments").update({ progress: pct })
  ...
  if (pct === 100) {
    ...
    if (!existing) {
      ...
      await (await admin()).from("certificates").insert({
...
export const getStudentCourse = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: completions } = lessonIds.length
      ? await (await admin()).from("lesson_completions").select("lesson_id")
...
export const getStudentLesson = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    const { data: lesson } = await (await admin()).from("lessons").select("*, sections!inner(course_id)").eq("id", data.lessonId).single();
    ...
    const { data: submission } = await (await admin()).from("activity_submissions")
      .select("*").eq("lesson_id", data.lessonId).eq("student_id", context.userId)
      .order("submitted_at", { ascending: false }).limit(1).maybeSingle();
    const { data: attempt } = await (await admin()).from("quiz_attempts")
      .select("*").eq("lesson_id", data.lessonId).eq("student_id", context.userId)
      .order("completed_at", { ascending: false }).limit(1).maybeSingle();
    const { data: completion } = await (await admin()).from("lesson_completions")
      .select("id").eq("lesson_id", data.lessonId).eq("student_id", context.userId).maybeSingle();
    let signedFile: string | null = null;
    if (submission?.file_url) {
      const { data: signed } = await (await admin()).storage.from("submissions")
...
export const markLessonComplete = createServerFn({ method: "POST" })
  ...
  .inputValidator((d: unknown) => z.object({ lessonId: z.string().uuid() }).parse(d))
  .handler(async ({ data, context }) => {
    const { data: lesson } = await (await admin()).from("lessons").select("section_id, sections!inner(course_id)").eq("id", data.lessonId).single();
    const courseId = (lesson as any).sections.course_id;
    await assertEnrolled(context.userId, courseId);
    await (await admin()).from("lesson_completions").upsert(
...
export const requestUploadUrl = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: signed, error } = await (await admin()).storage.from("submissions")
...
export const submitActivity = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const courseId = (lesson as any).sections.course_id;
    await assertEnrolled(context.userId, courseId);
    const { error } = await (await admin()).from("activity_submissions").insert({
...
export const submitQuiz = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { data: lesson } = await (await admin()).from("lessons").select("content, section_id, sections!inner(course_id)").eq("id", data.lessonId).single();
    const courseId = (lesson as any).sections.course_id;
    await assertEnrolled(context.userId, courseId);
...
export const getMyProgress = createServerFn({ method: "GET" })
  .middleware([requireSupabaseAuth])
  .handler(async ({ context }) => {
    const { data: enrollments } = await (await admin()).from("enrollments")
      .select("course_id, progress, courses(id,title)").eq("student_id", context.userId);
    const courseIds = (enrollments ?? []).map((e: any) => e.course_id);
    if (!courseIds.length) return [];
    const { data: sections } = await (await admin()).from("sections").select("*").in("course_id", courseIds).order("order_index");
    const sectionIds = (sections ?? []).map((s) => s.id);
    const { data: lessons } = sectionIds.length
      ? await (await admin()).from("lessons").select("*").in("section_id", sectionIds).order("order_index")
      : { data: [] as any[] };
    const lessonIds = (lessons ?? []).map((l) => l.id);
    const { data: completions } = lessonIds.length
      ? await (await admin()).from("lesson_completions").select("lesson_id").eq("student_id", context.userId).in("lesson_id", lessonIds)
      : { data: [] as any[] };
    const done = new Set((completions ?? []).map((c) => c.lesson_id));
