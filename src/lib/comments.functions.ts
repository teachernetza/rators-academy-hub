...
async function assertCanAccessLesson(userId: string, lessonId: string) {
  ...
  const [{ data: enr }, { data: course }, { data: prof }] = await Promise.all([
    supabaseAdmin.from("enrollments").select("id").eq("student_id", userId).eq("course_id", courseId).maybeSingle(),
...
export const postComment = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: row, error } = await supabaseAdmin.from("lesson_comments").insert({
...
export const deleteComment = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { data: row } = await supabaseAdmin.from("lesson_comments").select("user_id").eq("id", data.id).single();
    ...
    await supabaseAdmin.from("lesson_comments").delete().eq("id", data.id);
    return { ok: true };
  });
